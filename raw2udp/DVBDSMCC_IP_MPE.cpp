...
//
//////////////////////////////////////////////////////////////////////
//  CYJ,2008-12-8 修改 OnTableReceived, 支持 Session 拼接
...
DWORD DVB_GetCRC32( PBYTE pBuf, int nLen, UINT dwLastCRC)
{
...
CDVBDSMCC_IP_MPE::CDVBDSMCC_IP_MPE( CUDPDataPort * pDataPort, WORD wPID )
{
    ...
    m_pInputDataBuf = m_abyDataBuf + sizeof(ETHERNET_FRAME);	// 保留一个以太帧的位置
    m_nDataLen = 0;

    m_bHeaderReceived = false;			//	表头接收到了
    m_wErrorCounter = 0;				//	TS packet 发生错误的次数
    m_byExpectTSContinuityCounter = 0xff;	//	上次 TS packet 的continuity counter

    m_pPreItem = NULL;			// 上一个
    m_pNextItem = NULL;			// 下一个
}
...
void CDVBDSMCC_IP_MPE::PushOneTSPacket(PDVB_TS_PACKET pPacket)
{
    ...
    {							//	接收到一个新表
        ...
        if( m_bHeaderReceived && 0 == m_wErrorCounter && m_nDataLen > 0 )	// 没有发生错误，且成功接收到数据
        {
            if( OnTableReceived() )	//  CYJ,2008-12-8 判断是否连续Sesstion。通知接收到表
                Reset();
        }
        else
            Reset();				//	重新开始
...
/// 返回参数:
///		TRUE			需要执行 Reset 操作
///		FALSE			接收到一个新的Session，不能执行 Reset
BOOL CDVBDSMCC_IP_MPE::OnTableReceived()
{
    ...
    ASSERT( pSrcBuf );
#ifdef ULE
    if( NULL == pSrcBuf || nByteReceived <= 4 )	// 一个PAT表至少需要4字节
        return TRUE;

    WORD wType = (pSrcBuf[2]<<8)|pSrcBuf[3];//ipv4:0x0800 ipv6:0x86dd
    if( wType != 0x0800 && wType != 0x86dd )
    {
#ifdef __ENABLE_TRACE__
        TRACE("CDVBPSITable_IP_MPE: Not a ULE private data table, wType=0x%04x \n", wType );
#endif // __ENABLE_TRACE__
        return TRUE;
    }
    WORD wSectionLen = ((pSrcBuf[0]&0x7F) << 8) | pSrcBuf[1];
    if( nByteReceived < wSectionLen+2 )
    {
#ifdef __ENABLE_TRACE__
    ...
    bool bDestinationAddressAbentField = ( pSrcBuf[0] & 0x80 );
    if( DVB_GetCRC32( pSrcBuf, wSectionLen+2, 0xffffffff ) )
    {							//	包括CRC32在内，整个结构大小＝wSectionLen＋2。
#ifdef __ENABLE_TRACE__
        TRACE("CDVBPSITable_IP_MPE: Bad CRC32\n");
#endif // __ENABLE_TRACE__
        return  TRUE;
    }
    PBYTE pDataPtr = pSrcBuf + 4;
    WORD wDataLen = wSectionLen - 2 - 4;	// 4 bytes CRC32 or checksum, but 3 bytes header(table_id and senction len)
    if( false == bDestinationAddressAbentField )
    {
        m_abyEthAddr[5] = pSrcBuf[5];
        m_abyEthAddr[4] = pSrcBuf[4];
        m_abyEthAddr[3] = pSrcBuf[3];
        m_abyEthAddr[2] = pSrcBuf[2];
        m_abyEthAddr[1] = pSrcBuf[1];
        m_abyEthAddr[0] = pSrcBuf[0];
...
#endif // __ENABLE_TRACE__
        return TRUE;						//	不是 MPE private data 表
    }
    ...
    {
        if( DVB_GetCRC32( pSrcBuf, wSectionLen+3, 0xffffffff ) )
    ...
    if( 0 == pSrcBuf[6] )		// only one section
    {
        ...
        //  CYJ,2008-12-8 发现剩余数据是一个新的数据表，继续接收。
...
bool CDVBDSMCC_IP_MPE::IsPSIPacketIntegral()
{
    ...
    ASSERT( nByteReceived );
#ifdef  ULE
    if( nByteReceived <= 4+m_nDataHeaderLen )	// 一个MPE表至少需要4字节，再加20字节的IP头
        return false;
    WORD wSectionLen = ((m_pInputDataBuf[0]&0x7f) << 8) | m_pInputDataBuf[1];
    return( nByteReceived >= wSectionLen+2 );
#else
    if( nByteReceived <= 12+m_nDataHeaderLen )	// 一个MPE表至少需要12字节，再加20字节的IP头
        return false;
...
inline void CDVBDSMCC_IP_MPE::OnEthernetFrame( PBYTE pEthernetAddr, int nFrameLen )
{
    ...
    int headLen = sizeof(ETHERNET_FRAME) + nIPLen + 8;

    sendLen = m_pDataPort -> SentData( pEthernetAddr + headLen, nFrameLen - headLen );
    //  cout << "nFrameLen = " << nFrameLen - headLen << ", sendLen = " << sendLen << endl;
}
