...
#define IN_WIDTH	320//640
#define IN_HEIGHT	240//480
#define OUT_WIDTH	320//640//320
#define OUT_HEIGHT	240//480//240
...
//	printf("pi8BMp = 0x%08x\n",pi8BMP);
//	printf("pi8BMp = 0x%08x\n",*(pi8BMP+1));
//    ioctl(gpio_fd, IOCTL_READ_DATA, 10);
...
static void mainloop(void)
{
	...
	char filename[256];
	while(1) {
		sprintf(filename,"/usr/image%02d",cnt++);
...
static void stop_capturing(void)
{
    ...
    ioctl(gpio_fd, IOCTL_CAP_VH, 10);    // Vsync set high
...
static void start_capturing(void)
{
    ...
    ioctl(gpio_fd, IOCTL_READ_REG, 10);

    ioctl(gpio_fd, IOCTL_CAP_VL, 10);    // Vsync set low
}
...
static void init_device(void)
{
	...
	ioctl(gpio_fd, IOCTL_LED_ON, 10);    // 点亮led
	ioctl(gpio_fd, IOCTL_CAP_VH, 10);    // Vsync set high
...
static void open_device(void)
{
	...
	gpio_fd = open(gpio_dev, 0);  // 打开设备
    if (gpio_fd < 0) {
        printf("Can't open /TS_GPIO\n");
        exit(EXIT_FAILURE);
    }
}
...
{
	dev_name = "/dev/video0";
	gpio_dev = "/dev/TS_GPIO";
	dev_fmt = V4L2_PIX_FMT_GREY;//V4L2_PIX_FMT_JPEG;////V4L2_PIX_FMT_RGB565;
