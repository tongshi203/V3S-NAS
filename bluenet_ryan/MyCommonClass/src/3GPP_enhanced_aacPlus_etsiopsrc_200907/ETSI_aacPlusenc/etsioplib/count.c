 ...
 *   $Id: count.c,v 1.9 2005-03-08 17:42:14 ehr Exp $
 *************************************************************************/
...
#define FRAME_LEN   2048.0f
#define WMOPS_NORM_FAC (SAMPLE_RATE/FRAME_LEN/1000000.0f)
...
void Reset_WMOPS_counter (void)
{
    ...
    total_wmops[currCounter] += ((float) tot) * WMOPS_NORM_FAC;
...
void WMOPS_output (Word16 dtx_mode)
{
     ...
     */
    tot = WMOPS_frameStat();
    tot_wm = (Word32) (total_wmops[currCounter] + (float) (tot) * WMOPS_NORM_FAC);

    fprintf (stdout, "%10s:WMOPS=%.3f",
	     objectName[currCounter]?objectName[currCounter]:"",
	     ((float) tot) * WMOPS_NORM_FAC);
    ...
    fprintf (stdout, "  WorstCase=%.3f",
             ((float) glob_wc[currCounter]) * WMOPS_NORM_FAC);
    ...
    if (dtx_mode == 0)
    {
        ...
        fprintf (stdout, "  WorstWC=%.3f", ((float) tot_wc) * WMOPS_NORM_FAC);
    }
...
void WMOPS_summary (Word16 noWorstCase)
{
#if WMOPS
  int i;
  float total_WMOPS = 0;
  ...
  fprintf(stdout,"\n\nSummary over all counters:");
  
  for (i=1; i<=maxCounter; i++) {
    if (nbframe[i] != 0) {
      fprintf (stdout, "\n%10s \tWMOPS Average =\t%.3f",
               objectName[i]?objectName[i]:"",
               total_wmops[i] / (float) nbframe[i]);
      total_WMOPS +=  total_wmops[i] / (float) nbframe[i];
    }
    
  }
  fprintf(stdout,"\n\nSummary total average WMOPS: %.2f\n", total_WMOPS);
#endif
}
