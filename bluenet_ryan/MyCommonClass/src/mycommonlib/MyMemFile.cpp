...
bool CMemFile::GrowFile(DWORD dwNewLen)
{
...
bool CMemFile::SetLength( unsigned long dwNewLen )
{
	if (dwNewLen > m_nBufferSize)
    {
		if( !GrowFile(dwNewLen) )
        	return false;
    }
...
unsigned int CMemFile::Write(const void* lpBuf, unsigned int nCount)
{
	if (nCount == 0)
		return 0;
    ...
    {
		if( !GrowFile(m_nPosition + nCount) )
        	return 0;
    }
...
bool CMemFile::SeekToBegin()
{
    Seek( 0, begin );
	return true;
}
long CMemFile::SeekToEnd()
{
    return Seek( 0, end );
}
...
void CMemFile::Close()
{
	ASSERT((m_lpBuffer == NULL && m_nBufferSize == 0) || (!m_bAutoDelete || m_lpBuffer) );
	...
	*ppBufStart = m_lpBuffer + m_nPosition;
	...
	else
	{
		...
		*ppBufMax = m_lpBuffer + min(m_nFileSize, m_nPosition + nCount);
		m_nPosition += PBYTE(*ppBufMax) - PBYTE(*ppBufStart);
	}

	// return number of bytes in returned buffer space (may be <= nCount)
	return PBYTE(*ppBufMax) - PBYTE(*ppBufStart);
}

unsigned long CMemFile::GetLength()
{
 	return m_nFileSize;
}
