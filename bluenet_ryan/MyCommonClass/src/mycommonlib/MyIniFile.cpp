...
// If do the test, please define below macro
// #define INIFILE_TEST_THIS_FILE

#if defined( INIFILE_TEST_THIS_FILE ) && defined( _WIN32 )
...
BOOL CIniFile::OpenIniFile (CCHR * FileName)
{
    ...
    CMyString strTmp;
	while (fgets( strTmp.GetBuffer(300), 255, m_pIniFile) != NULL )
	{
		strTmp.TrimLeft();
        strTmp.TrimRight();
		pEntry = MakeNewEntry ();
		...
		pEntry->pText = (char *)malloc (strTmp.GetLength()+1);
		if (pEntry->pText == NULL)
		{
			FreeAllMem ();
			return FALSE;
		}
		strcpy (pEntry->pText, strTmp);

		int nPos = strTmp.Find( ';' );
        if( nPos >= 0 )
        	strTmp.ReleaseBuffer( nPos );		// cut comment

        if( strTmp.Find('[') >= 0 && strTmp.Find(']') >= 0 )	/* Is Section */
		{
	...
	while (pEntry != NULL)
	{
		...
		if (pEntry->Type == tpKEYVALUE)
		{
			...
			if (pText != NULL)
			{
				...
				*pText = '=';
				/*            printf ("%s,%s\n", Search, Found); */
				if( Found.CompareNoCase( Search ) == 0 )
				{
	               strcpy( pList->KeyText, Found );
                   pText ++;
                   while( *pText && *pText == 0x20 )
                   	  pText ++;			// bypass blank char
				   strcpy( pList->ValText, pText );
				   pList->pKey = pEntry;
				   return TRUE;
				}
			}
