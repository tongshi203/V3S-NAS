...
//////////////////////////////////////////////////////////////////////////
#define X264_RATECTRL_ENTRY_COUNT  128		// 必须是 2 的 N 次方
...
#define BITRATE_WINDOW_SIZE 32
	...
	float	m_fLongTimeAverageTotalByteInWindow;	// 长时间的码率平均

	int		m_nBitsPerFrame;						// 每帧允许的比特数
	int		m_nTotalICountInWindow;					// 窗口中 I 宏块个数

#define Q_SUM_WINDOWS_SIZE 16
	int		m_nQSum_Windowed;						// 在窗口中的总Q值
	int		m_nBitsOneSecond;						// 一秒内的比特总数
	int		m_nAverageBitrateOverflow;				// 平均比特率溢出
	float	m_fAverageBitRateChangingRange;			// 变化幅度，0 ~ 1
	int		m_nLastIPQOffset;						// 上次的 I/P 帧 量化系数
};
...
/// 返回参数:
///		无
inline ratecontrol_entry_t * GetRateCtrlEntryAt( unsigned int nFrameIndex )
{
...
#define ABR_INIT_QP ( h->param.rc.i_rc_method == X264_RC_CRF ? h->param.rc.f_rf_constant : 24 )
    ...
    *rc->pred_b_from_p = rc->pred[0];
	...
	rc->m_fLongTimeAverageTotalByteInWindow = rc->m_dwTotalBytesWindowed;

	rc->m_adwEachFrameBytesWindowed[ 0 ] = rc->bitrate / ( rc->fps * 8 );
	rc->m_nFrameWindowPtr = 0;
	rc->m_fLastQP_IP = 26;
	rc->m_fAverageQP = 26;
	rc->m_nQSum_Windowed = rc->m_fAverageQP * Q_SUM_WINDOWS_SIZE * 2;
	rc->m_afUpperBPSLimit[0] = 1.5f;		// 一半水平
	rc->m_afUpperBPSLimit[1] = 1.8f;
	rc->m_nBitsPerFrame = rc->bitrate / rc->fps;
	rc->m_nBitsOneSecond = rc->bitrate;
	rc->m_nTotalICountInWindow = 0;
	rc->m_nAverageBitrateOverflow = 0;
	rc->m_fAverageBitRateChangingRange = 0;
	rc->m_nLastIPQOffset = 0;
	...
	if( h->param.rc.b_stat_write )
	{
		...
		for(i=0; i<X264_RATECTRL_ENTRY_COUNT; i++ )
		{
			...
			s_aRateCtrlEntry[i].new_qp = rc->m_fAverageQP;
		}
	}
...
#define GET_TARGET_BIT_FRAME_COUNT	64
	...
	nTargetBits = (int)( nBitAvaible * ((float)rce->m_nTotalBits/nTotalBits*2) );
...
void x264_ratecontrol_start( x264_t *h, int i_force_qp )
{
    ...
    else if( rc->b_abr )
    {
		...
		{												//  最大 1.25；在 80% 左右是不做调整；超过 80% 增大；反之降低
			q = ( (rc->m_fAverageQP*3 + rc->m_fLastQP_IP)/4 ) * pow( (1 + fAverageRate + fRate/4 + rc->m_fAverageBitRateChangingRange/2 )/2, 0.5f );
    ...
    else if( rc->b_2pass )
    {
		...
		if( h->sh.i_type != SLICE_TYPE_B )
		{
			...
			fRate = ( ( fRate*pow(fAverageRate/0.85,2) + 3 )*fAverageRate + fFutureRate + rce->m_fAverageBufUsage ) / 6;
			...
			else if( fAverageRate < 0.75f )
			{
				rc->m_nAverageBitrateOverflow = 0;
				Q_Offset = 2;
			}
			else if( 0 == rc->m_nAverageBitrateOverflow && fAverageRate < 0.875f )
				Q_Offset = 3;
			else
			{
				if( fAverageRate > 0.92f )		// 一旦超过 98% 就算溢出；然后一直到 85% 以下，才算不溢出；迟滞曲线
					rc->m_nAverageBitrateOverflow = 5;		// 连续5次以后，才认为真的回到了低缓冲区的样子
				else if( fAverageRate < 0.8f && rc->m_nAverageBitrateOverflow )
					rc->m_nAverageBitrateOverflow --;

				if( rc->m_nAverageBitrateOverflow )
					Q_Offset = 6 + (int)( pow( fAverageRate+0.1, 6 ) * 4 );
				else
					Q_Offset = 4;
			}
			Q_Offset = ( rc->m_nLastIPQOffset*3 + Q_Offset ) / 4;
			rc->m_nLastIPQOffset = Q_Offset;
			...
			q = ( rce->m_fAverageQ + rc->m_fAverageQP * 3 + rc->m_fLastQP_IP/2 + rce->new_qp ) / 5.5f;
			fWeight = fRate + (rc->m_fAverageBitRateChangingRange)/1.5 + (float)rc->m_nAverageBitrateOverflow/30;
			q = q * pow( fWeight, 0.5 );
			...
			if( h->sh.i_type == SLICE_TYPE_I )
				q = x264_clip3( (int)(q + 0.5), 14+Q_Offset/2, 24+rc->m_nOverflowTimes+Q_Offset/2 );
			else
				q = x264_clip3( (int)(q + 2.5), 16+Q_Offset, 32+rc->m_nOverflowTimes+Q_Offset );
...
void x264_ratecontrol_end( x264_t *h, int bits )
{
	...
	nLastOneSecPtr = rc->m_nFrameWindowPtr - rc->fps;
	if( nLastOneSecPtr < 0 )
		nLastOneSecPtr += BITRATE_WINDOW_SIZE;
	rc->m_nBitsOneSecond += ( bits - rc->m_adwEachFrameBytesWindowed[ nLastOneSecPtr ]*8 );
	...
	rc->m_fLongTimeAverageTotalByteInWindow = ( rc->m_fLongTimeAverageTotalByteInWindow * 127 + rc->m_dwTotalBytesWindowed ) / 128;

	if( rc->m_fLongTimeAverageTotalByteInWindow > 1 )
	{
		fChangedRange = sqrt( rc->m_fAverageTotalByteWindowed / rc->m_fLongTimeAverageTotalByteInWindow );
		fChangedRange = fabs( fChangedRange - 1 );
	}
	else
		fChangedRange = 0;
	rc->m_fAverageBitRateChangingRange = ( rc->m_fAverageBitRateChangingRange * 15 + fChangedRange ) / 16;
	//}}}
	...
	if( h->fenc->i_frame >= rc->fps )
		rc->m_nTotalICountInWindow -= GetRateCtrlEntryAt( h->fenc->i_frame - rc->fps )->i_count;
	rc->m_nTotalICountInWindow += h->stat.frame.i_mb_count_i;

	// 保存平均的 Q 系数
	pRCE = GetRateCtrlEntryAt( h->fenc->i_frame - Q_SUM_WINDOWS_SIZE );
	pRCE->m_fAverageQ = (float)rc->m_nQSum_Windowed / (2*Q_SUM_WINDOWS_SIZE);
	pRCE->m_fAverageBufUsage = (float)rc->m_dwTotalBytesWindowed / rc->m_dwExpectBytesWindowsed;
	pRCE->m_nTotal_I_Count = rc->m_nTotalICountInWindow;
	if( 0 == pRCE->m_nTotal_I_Count )
		pRCE->m_nTotal_I_Count = 1;
	rc->m_nQSum_Windowed -= GetRateCtrlEntryAt( h->fenc->i_frame - Q_SUM_WINDOWS_SIZE*2 )->new_qp;
	rc->m_nQSum_Windowed += rc->qpa;
    ...
    if( h->param.rc.b_stat_write )
    {
        ...
        char c_direct = h->mb.b_direct_auto_write ?
                        ...
                        : '-';
		//  CYJ,2007-12-13 Add
		pRCE = GetRateCtrlEntryAt( h->fenc->i_frame );
		...
		pRCE->m_fBitrate = (float)rc->m_nBitsOneSecond/1000;
	...
	if( rc->p_stat_file_out )
	{
		if( rc->b_2pass )
		{
			...
			fprintf( rc->p_stat_file_out,
					 "f:%6d t:%c q:%5d it:%5d pt:%5d mv:%5d mi:%5d im:%5d pm:%5d sm:%5d d:%c br:%6.1f bu:%5.1f%% ab:%5.1f%% aq:%5.1f tbu:%3d%%;\n",
		...
		else if( h->fenc->i_frame > BITRATE_WINDOW_SIZE )
		{
			char c_type;
			char c_direct;

			pRCE = GetRateCtrlEntryAt( h->fenc->i_frame - BITRATE_WINDOW_SIZE );
			c_type = pRCE->pict_type==SLICE_TYPE_I ? (pRCE->kept_as_ref ? 'I' : 'i')
						: pRCE->pict_type==SLICE_TYPE_P ? 'P'
						: pRCE->kept_as_ref ? 'B' : 'b';
			c_direct = pRCE->direct_mode ? pRCE->direct_mode : ' ';

			fprintf( rc->p_stat_file_out,
					 "f:%6d t:%c q:%5d it:%5d pt:%5d mv:%5d mi:%5d im:%5d pm:%5d sm:%5d d:%c br:%6.1f bu:%5.1f%% ab:%5.1f%% aq:%5.1f tbu:%3d%%;\n",
					 h->fenc->i_frame - BITRATE_WINDOW_SIZE,
					 c_type,			pRCE->new_qp,
					 pRCE->i_tex_bits,	pRCE->p_tex_bits,	pRCE->mv_bits,	pRCE->misc_bits,
					 pRCE->i_count,		pRCE->p_count,		pRCE->s_count,	c_direct,
					 pRCE->m_fBitrate,
					 (float)rc->m_dwTotalBytesWindowed*100/rc->m_dwExpectBytesWindowsed,
					 pRCE->m_fAverageBufUsage * 100,
					 pRCE->m_fAverageQ,
					 s_nTotalBitsInRateCtrlEntry * 100 / s_nExpectBitsInRateCtrlEntry );
		}
	}
...
{
	x264_ratecontrol_t *rc = h->rc;
	long nOldVal = rc->bitrate;
	rc->bitrate = nNewBitrateKBPS * 1000;
	rc->wanted_bits_window = 1.0 * rc->bitrate / rc->fps;

	// 窗口中的总字节数
	int nNewExpectBytesWindowed = ( rc->m_dwExpectBytesWindowsed + (BITRATE_WINDOW_SIZE * rc->bitrate * 3) / ( rc->fps * 8 ) ) / 4;
	rc->m_dwExpectBytesWindowsed = nNewExpectBytesWindowed;
