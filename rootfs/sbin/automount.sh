...
if [ "$ACT" = "Xremove" ]; then
	...
	if [ $(echo $1 | grep mtd) ]; then
		...
		if mount -t vfat -o noatime,shortname=mixed,utf8 "/dev/$1" "/mnt/$1"; then
			...
			echo "$ACT /mnt/$1" >> /tmp/mdev.log
		elif mount -t exfat "/dev/$1" "/mnt/$1"; then
			echo "[Mount exFAT]: /dev/$1 --> /mnt/$MNT" > /dev/console
			echo "$ACT /mnt/$1" >> /tmp/mdev.log
		elif mount -t ext2 "/dev/$1" "/mnt/$1"; then
                        echo "[Mount ext2]: /dev/$1 --> /mnt/$MNT" > /dev/console
                        echo "$ACT /mnt/$1" >> /tmp/mdev.log
		else
			# failed to mount, clean up mountpoint
			if ! rmdir "/mnt/$1"; then
